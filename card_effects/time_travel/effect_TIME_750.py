...
def on_play(game: Game, player: Player, card: Card, target=None):
    ...
    has_big_minion = any(c.card_type == CardType.MINION and game.get_card_cost(player, c) >= 5 for c in player.hand if c != card)
    
    if has_big_minion:
        # Draw a minion
        minions_in_deck = [c for c in player.deck if c.card_type == CardType.MINION]
        if minions_in_deck:
